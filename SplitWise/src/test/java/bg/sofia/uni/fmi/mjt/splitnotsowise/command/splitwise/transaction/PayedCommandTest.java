...
class PayedCommandTest {
    ...
    @Test
    void testPayedNotLoggedIn() {
        logoutCommand.execute(logger);
        String[] args = "payed 5 bill clinton".split(SPACE_DELIMITER);
        PayedCommand payedCommand = new PayedCommand(args, ADMIN_ADDRESS);
        payedCommand.execute(log);
        verify(log, times(1)).log(anyString(),any());
    }

    @Test
    void testPayedIncorrectAmount() {

        String[] args = "payed 5d bill clinton".split(SPACE_DELIMITER);
        PayedCommand payedCommand = new PayedCommand(args, ADMIN_ADDRESS);
    ...
    @Test
    void testPayedNegativeAmount() {

        String[] args = "payed -5 friend1 beans".split(SPACE_DELIMITER);
        PayedCommand payedCommand = new PayedCommand(args, ADMIN_ADDRESS);
    ...
    @Test
    void testPayedNonFriend() {

        String[] args = "payed 5 friend2 beans".split(SPACE_DELIMITER);
        PayedCommand payedCommand = new PayedCommand(args, ADMIN_ADDRESS);
